use glium::{backend::Facade, implement_vertex, VertexBuffer};
...
impl Vertex {
    /// Creates the Vertices, which will result in a Rectangle, if drawn as
    /// triangle strip The coordinates are normalized to a range from 0 to 1
